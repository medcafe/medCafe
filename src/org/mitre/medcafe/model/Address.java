	...
	public JSONObject saveAddress(String patientId, JSONObject addresses) {
		...
		try {
			...
			if (rtn < 0) {
				...
				dbConn.close();
				return WebUtils
						.buildErrorJson("Problem on deletion of current data from database .");
			}

			if (addresses == null)
				log.fine("Address: saveAddress : address is null ");
			...
			log.severe("Address: saveAddress : problem on SQL  "
					+ e.getMessage());
			return WebUtils
					.buildErrorJson("Problem on selecting data from database for address ."
							+ e.getMessage());
			...
			e.printStackTrace();
			return WebUtils
					.buildErrorJson("Problem on processing JSON Object for address ."
							+ addresses.toString() + " " + e.getMessage());
		} finally {
