...
public class Patient {
	public final static String	KEY									= Patient.class
																			.getName();
	...
	private String				medcafe_photo						= "photo_1.jpg";
	...
	public static final String	SEARCH_PATIENTS_BY_ID				= "SELECT DISTINCT id, first_name, last_name, photo, patient.repository from patient where  patient.id = ?";
	...
	public static final String	INSERT_PATIENT						= "INSERT INTO patient (first_name, last_name, rep_patient_id, photo) values (?,?,?,?) ";
	...
	public Patient(String repo_name){
		this();
		this.primaryRepos = Repositories.getRepository(repo_name);
	}
	...
	public JSONObject associatePatient(String userName, int patient_id,
			String role) {
		...
		try {
			...
			// INSERT_ASSOCIATION =
			// "INSERT INTO patient_user_assoc (patient_id, user_id, role) values (?,?,?) ";
			String insertQuery = INSERT_ASSOCIATION;
			String err_mess = "Could not insert association for patient  "
					+ patient_id;
			if (role == null) role = "physician";
			...
			return WebUtils
					.buildErrorJson("Problem accessing database for patient "
							+ patient_id);
		} finally {
		...
		return WebUtils
				.buildErrorJson("Problem on creating an association for patient."
						+ patient_id); }
	...
	public JSONObject associatePatientRepository(String patientId,
			String patient_rep_id, String repository, String userName) {
		JSONObject ret = new JSONObject();
		DbConnection dbConn = null;
		try {
			dbConn = setConnection();
			...
			if (patientId == null) {
				...
				log.severe("Patient: associatePatientRepository patient exists already JSON data  "
						+ ret.toString());

				if (ret.get("exists").equals("true")) return ret;
				log.setLevel(Level.ALL);
				// First create a new patient
				ret = insertPatient(patient_rep_id);
				log.finer("Patient: associatePatientRepository got JSON Object "
						+ ret.toString());
				patient_id = ret.getInt(Patient.ID);
			...
			ret = associatePatient(userName, patient_id, "physician");

			// INSERT_ASSOCIATION =
			// "INSERT INTO patient_user_assoc (patient_id, user_id, role) values (?,?,?) ";
			String insertQuery = INSERT_REPOSITORY_ASSOCIATION;
			int rtn = 0;
			String err_mess = "Could not insert repository association for patient  "
					+ patient_id;

			// public static final String INSERT_REPOSITORY_ASSOCIATION =
			// "INSERT INTO patient_repository_assoc (patient_id, rep_patient_id,repository) values (?,?,?) ";

			rtn = dbConn.psExecuteUpdate(insertQuery, err_mess, patient_id,
					patientRepId, repository);

			if (rtn < 0) { return WebUtils
					.buildErrorJson("Problem on creating an association for patient."
							+ patient_id); }
			ret = checkExists(patient_rep_id, repository);
			log.finer("Patient: associatePatientRepository patient added successfully "
					+ ret.toString());
			...
			e.printStackTrace();
			return WebUtils
					.buildErrorJson("Error on retrieving patient JSON data "
			...
			e.printStackTrace();
			return WebUtils
					.buildErrorJson("Error on saving patient association information "
							+ firstName + " " + lastName + " " + e.getMessage());
		} finally {
			closeConnections(dbConn);
			dbConn = null;
	...
	public JSONObject checkExists(String patientRepId, String repository) {
		...
		try {
			...
			String err_mess = "Could not check for existence of patient  "
					+ firstName + " " + lastName;

			// First check that this patient has not already been added
			String checkPatient = SEARCH_PATIENTS_REP_ASSOC_BY_REP_ID;
			rs = dbConn.psExecuteQuery(checkPatient, err_mess, patient_rep_id,
					repository);
			...
			return WebUtils.buildErrorJson("Error on retrieving patient data "
					+ firstName + " " + lastName + " " + e.getMessage());

		} catch (JSONException e) {

			// TODO Auto-generated catch block
			return WebUtils.buildErrorJson("Error on generating JSON data "
					+ firstName + " " + lastName + " " + e.getMessage());
	...
	public JSONObject insertPatient(String patientRepId) {
		...
		// It's a new patient! Might as well try to get a photo out of the
		// repo if possible!
		try {
			this.medcafe_photo = this.primaryRepos.downloadPhoto(patientRepId);
		} catch (Exception e1) {
			log.warning("Couldn't download photo for patient: " + patientRepId);
		}
		...
		try {
			...
			Integer patient_rep_id =  Integer.parseInt(patientRepId);

			String err_mess = "Could not insert patient  " + firstName + " "
					+ lastName;

			int rtn = dbConn.psExecuteUpdate(INSERT_PATIENT, err_mess,
					firstName, lastName, patient_rep_id, this.medcafe_photo);

			if (rtn < 0) throw new Exception("Adding patient to medcafe db failed");

			log.info("Added new patient to medcafe DB: " + lastName + ", " + firstName);
			
		    // Now get the newly inserted patient
			
			rs = dbConn.psExecuteQuery(SEARCH_PATIENT, err_mess, lastName,
					firstName, patient_rep_id);

			if ( ! rs.next() ) { // couldn't find the patient we just inserted!	
				throw new Exception("Could not find patient with name  :"
						+ firstName + " " + lastName);
			}
		...
		} catch (Exception e) { //TODO: throw from here
			e.printStackTrace();
			return WebUtils.buildErrorJson("Error on retrieving patient data "
					+ firstName + " " + lastName + " " + e.getMessage());
		} finally {
			closeConnections(dbConn, rs); //free..
		}
	}
	...
	public JSONObject isPatient(String userName, String patientId) {
		...
		String err_mess = "Could not check if there is an existing association for patient  "
				+ patient_id;
		try {
			dbConn = setConnection();
			rs = dbConn.psExecuteQuery(checkQuery, err_mess, patient_id,
					userName);
			...
			return WebUtils
					.buildErrorJson("Problem on checking if patient currently is in list. "
							+ patient_id + " Error " + e.getMessage());
		} catch (JSONException e) {

			// TODO Auto-generated catch block
			return WebUtils
					.buildErrorJson("Problem on JSON creation on checking if patient currently is in list. "
							+ patient_id + " Error " + e.getMessage());
		} finally {
			closeConnections(dbConn, rs);
			dbConn = null;
	...
	public JSONObject isPatient(String patientId, String patientRepId,
			String repository) {
		DbConnection dbConn = null;
		ResultSet rs = null;
		JSONObject ret = new JSONObject();
		...
		String err_mess = "Could not check if there is an existing repository association for patient  "
				+ patient_id;
		try {
			dbConn = setConnection();
			rs = dbConn.psExecuteQuery(checkQuery, err_mess, patient_id,
					patient_rep_id, repository);
	...
	public JSONObject listRepositories(String patientId) {
		...
		ResultSet rs = null;
		JSONObject ret = new JSONObject();
		// SELECT patient_id, rep_patient_id, repository from
		// patient_repository_assoc where patient_id = ? ";

		String listQuery = LIST_REP_ASSOC_BY_PATIENT_ID;

		int patient_id = Integer.parseInt(patientId);

		String err_mess = "Could not check the list of repositories for patient "
				+ patient_id;
		...
		try {
			...
			/*
			 * { "repositories" : [
			 * {"repository" : "OurVista" , "id" : "3"},{"repository" : "local"
			 * , "id" : "1"}
			 * ]
			 * }
			 */
			...
			return WebUtils
					.buildErrorJson("Problem on selecting data from database ."
							+ e.getMessage());
		...
		return WebUtils
				.buildErrorJson("There are no patients currently listed in repositories ");
	...
	public JSONObject searchJson(String isPatient, String searchStringFirst,
			String searchStringLast, String userName, String server) {
		DbConnection dbConn = null;
		ResultSet rs = null;
		...
		try {
			dbConn = setConnection();
			StatementResult stmtRes = getPatients(isPatient, searchStringFirst,
					searchStringLast, userName, server, dbConn);
			rs = stmtRes.getRS();
			prep = stmtRes.getPrep();
			if (rs == null) { return WebUtils
					.buildErrorJson("Could not establish a connection to the database  at this time."); }
			while (rs.next()) {
				// convert to JSON
				rtnResults = true;
			...
			try {
				...
				// TODO Auto-generated catch block
				return WebUtils
						.buildErrorJson("Problem on generating JSON error."
								+ e.getMessage());
			}
		} catch (SQLException e) {
		...
		return WebUtils
				.buildErrorJson("There are no patients currently listed for First Name "
						+ searchStringFirst
						+ " and Last Name "
						+ searchStringLast); }
	...
	private StatementResult getPatients(String isPatient,
			String searchStringFirst, String searchStringLast, String userName,
			String server, DbConnection dbConn) throws SQLException {
		...
		if (isPatient.equals("true")) {
			...
			if (searchStringFirst.length() == 0) {
				// prep =
				// dbConn.prepareStatement(Patient.SEARCH_PATIENTS_BY_LAST_NAME);
				...
				// prep =
				// dbConn.prepareStatement(Patient.SEARCH_PATIENTS_BY_FIRST_NAME);
				...
				// prep =
				// dbConn.prepareStatement(Patient.SEARCH_PATIENTS_BY_ALL);
	...
	public static JSONObject getPatient(int id) {
		...
		try {
			...
			if (!rtnResults) {
				...
				prep = null;
				rs = null;
				log.warning("There are no patients currently listed for patient id "
						+ id);
				return WebUtils
						.buildErrorJson("There are no patients currently listed for patient id "
								+ id);
			}
		} catch (SQLException e) {

			// TODO Auto-generated catch block
			log.warning("Problem on selecting data from database ."
					+ e.getMessage());
			return WebUtils
					.buildErrorJson("Problem on selecting data from database ."
							+ e.getMessage());
			...
			return WebUtils.buildErrorJson("Problem on generating JSON error."
					+ e.getMessage());
	...
	public static JSONObject getRecentPatients(String userName) {
		...
		try {
			...
			if (!rtnResults) {
				...
				return WebUtils
						.buildErrorJson("There are no recent patients currently listed for user "
								+ userName);
	...
	public static int getLocalId(String repository, String rep_patient_id) {
		...
		try {
			...
			prep = dbConn
					.prepareStatement(Patient.SELECT_PATIENT_ID_FOR_REPOSITORY);
			...
			// TODO Auto-generated catch block
			log.log(Level.SEVERE, "Problem on selecting data from database ."
					+ e.getMessage());
	...
	public static JSONObject getHistory(String patientId, String category,
		...
		try {
			...
			if (startDate != null) {
				sql = sql + Patient.SELECT_PATIENT_HISTORY_EXT
						+ SELECT_PATIENT_HISTORY_ORDER_BY;
				prep = dbConn.prepareStatement(sql);
			...
			while (rs.next()) {
				// SELECT patient_id, history, category_id, history_date,
				// history_notes
				...
				if (history_note != null) o.put("note", history_note);

				if (history_date != null) o.put("date", history_date);
			...
			if (!rtnResults) {
				...
				rs = null;
				return WebUtils
						.buildErrorJson("There is no patient history listed for patient "
								+ patient_id);
			...
			// TODO Auto-generated catch block
			return WebUtils.buildErrorJson("Problem on building JSON Object ."
					+ e.getMessage());
		} finally {
	...
	public JSONObject toJSON(int newid, String first_name, String last_name)
	...
	public void setPrimaryRepos(Repository primaryRepos) {
