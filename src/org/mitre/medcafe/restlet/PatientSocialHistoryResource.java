...
public class PatientSocialHistoryResource extends ServerResource {
    ...
    public JsonRepresentation toJson()
    {
		...
		try{
	        List<SocialHistory> socialHistory = r.getSocialHistory(this.id);
	        ...
	        {
	            return new JsonRepresentation(WebUtils.buildErrorJson( "There is no social history currently listed for patient " + id + " in repository " + repository ));
	        }
	        //convert to JSON
		    try{
		        log.finer(WebUtils.bundleJsonResponse("socialhistory",socialHistory,repository,id).getText());
		        }
		        catch (IOException IOe)
		        {
		        	log.severe("PatientProblemResource: Couldn't print JSON");
		        } 
		        return WebUtils.bundleJsonResponse( "socialhistory", socialHistory, repository, id );
		    }
