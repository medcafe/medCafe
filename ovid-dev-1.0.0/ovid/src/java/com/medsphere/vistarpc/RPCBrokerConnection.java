...
public class RPCBrokerConnection implements RPCConnection {
//  Header chunk types
...
//  Parameter types
    ...
    private Logger logger = Logger.getLogger(RPCBrokerConnection.class);
    ...
    public RPCResponse execute(VistaRPC rpc) throws RPCException {
        String tag = "RPCBroker RPC " + rpc.toString();
        TimeKeeper tk = new TimeKeeper();
        try {
            try {
                if (RPCBrokerTimer.canLog()) {
                    tk.start(tag);
                }
            ...
            if (retVal==null && signedOn) {
                ...
                if (connect()) {
                    if (currentContext != null) {
                        String context = new String(currentContext);
                        currentContext = null;
                        setContext(context);
                    }
            ...
            try {
                if (RPCBrokerTimer.canLog()) {
                    tk.stop(tag);
                    RPCBrokerTimer.log(tk.getDisplayStringWithTimeFirst(tag));
                }
            } catch (TimeKeeperException e) {
                e.printStackTrace();
            }
    ...
    public static class RPCBrokerTimer {
        private static Logger logger = Logger.getLogger(RPCBrokerTimer.class);
        ...
        public static void stopAndLog(String tag) {
            if (canLog()) {
                try {
                    tk.stop(tag);
                    logger.trace(tk.getDisplayStringWithTimeFirst(tag));
                } catch (TimeKeeperException e) {
                    e.printStackTrace();
                }
            }
