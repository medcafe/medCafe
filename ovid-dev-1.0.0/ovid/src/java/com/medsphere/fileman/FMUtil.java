...
public class FMUtil {
    ...
    private static final Pattern fmDatePattern = Pattern.compile( "(\\d{3})(\\d{2})?(\\d{2})?\\.?(\\d{1,2})?(\\d{1,2})?(\\d{1,2})?");
    ...
    static public Date fmDateToDate( String FMDate ) throws ParseException {
        ...
        Integer month = ((m.group( 2 )==null) ? 1 : new Integer( m.group( 2 ) )) - 1;
        Integer day = ((m.group( 3 )==null) ? 1 : new Integer( m.group( 3 ) ));
        if (m.group( 4 )==null) {
            ...
            // The hours trailing 0 was dropped.
            if (m.group (4).length() == 1) {
                hour *= 10;
            }

            // The minutes trailing 0 was dropped.
            if (m.group(5) != null && m.group (5).length() == 1) {
                minute *= 10;
            }

            // The seconds trailing 0 was dropped.
    ...
    static public String dateToFMDate( Date date ) {
        ...
        StringBuffer result = new StringBuffer( fmDatePartFormat.format(datePart) );
