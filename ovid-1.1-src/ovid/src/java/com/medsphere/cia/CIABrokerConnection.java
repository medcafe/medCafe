...
public class CIABrokerConnection extends AbstractRPCConnection {

    private final static byte EOD = (byte) 255;
    private final static String HEADER = "{CIA}";
    private Logger logger = LoggerFactory.getLogger(CIABrokerConnection.class);
    ...
    public synchronized RPCResponse execute(VistaRPC rpc) throws RPCException {
        ...
        switch (rpc.getType()) {
            ...
            case ARRAY:
                if (result.isEmpty()) {
                    retVal = new RPCResponse(new String[0]);
                } else {
