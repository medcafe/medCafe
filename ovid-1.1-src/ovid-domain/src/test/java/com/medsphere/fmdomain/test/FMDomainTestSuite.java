...
public class FMDomainTestSuite {
    ...
    private RPCPooledConnectionFactory factory = null;
    private Logger logger = LoggerFactory.getLogger(FMDomainTestSuite.class);

    private final String host = "localhost";
    private final String port = "9201";
    private final String accessCode = "OV1234";
    private final String verifyCode = "OV1234!!";
    private final String fmResourceUser = FMUtil.FM_RPC_CONTEXT;
    private final String fmFilemanResource = FMUtil.FM_RPC_NAME;
...
//    private final String fmFilemanResource = FMUtil.FM_RPC_NAME;
    ...
    public void setUp() {
        ...
        try {
            factory = new RPCBrokerPooledConnectionFactory(host, port, accessCode, verifyCode);
        } finally {}
    }
   ...
   @AfterClass
    ...
    @Test
    private void testFMQueryByIEN() throws RPCException, OvidDomainException {
        RPCPooledConnection connection = factory.getConnection();

        connection.setContext(fmResourceUser);
        RPCResAdapter adapter = new RPCResAdapter( connection, FMUtil.FM_RPC_NAME );
        FMFile f = new FMFile("PATIENT");
        f.addField("NAME");
        f.addField("SEX", FIELDTYPE.SET_OF_CODES);
        f.addField("DATE OF BIRTH", FIELDTYPE.DATE);
        FMQueryByIENS query = new FMQueryByIENS( adapter, f );
        query.setIENS(new String[] {"1","2","3"});
        try {
            FMResultSet results = query.execute();
            Assert.assertNotNull(results);
            Assert.assertTrue(results.next());
            Assert.assertEquals(results.getValue("IEN"), "1");
            System.out.println(
                    results.getValue("IEN")
                    + " : " + results.getValue("NAME")
                    + " : " + results.getValue("SEX")
                    + " : " + results.getValue("DATE OF BIRTH")
                    );
            Assert.assertTrue(results.next());
            Assert.assertEquals(results.getValue("IEN"), "2");
            System.out.println(
                    results.getValue("IEN")
                    + " : " + results.getValue("NAME")
                    + " : " + results.getValue("SEX")
            ...
            Assert.assertTrue(results.next());
            Assert.assertEquals(results.getValue("IEN"), "3");
            System.out.println(
                    results.getValue("IEN")
                    + " : " + results.getValue("NAME")
                    + " : " + results.getValue("SEX")
    ...
    @Test
    private void testFMQueryByIENWithAnnotatedClass() throws RPCException, OvidDomainException {
        RPCPooledConnection connection = factory.getConnection();

        connection.setContext(fmResourceUser);
        RPCResAdapter adapter = new RPCResAdapter( connection, FMUtil.FM_RPC_NAME );

        FMQueryByIENS query = new FMQueryByIENS( adapter, FMDrug.getFileInfoForClass() );
        query.setIENS(new String[] {"1","2","3"});
        try {
            FMResultSet results = query.execute();
            Assert.assertNotNull(results);
            ...
            FMDrug drug = new FMDrug(results);
            Assert.assertNotNull(drug);
            Assert.assertEquals(drug.getIEN(), "1");
            System.out.println(drug.getIEN()
            		           + " : " + drug.getGenericName()
            		           + " : " + drug.getNdc()
            		           );
            
            Assert.assertTrue(results.next());
            drug = new FMDrug(results);
            Assert.assertNotNull(drug);
            Assert.assertEquals(drug.getIEN(), "2");
            System.out.println(drug.getIEN()
            		           + " : " + drug.getGenericName()
            		           + " : " + drug.getNdc()
            		           );
            ...
            drug = new FMDrug(results);
            Assert.assertNotNull(drug);
            Assert.assertEquals(drug.getIEN(), "3");
            System.out.println(drug.getIEN()
            		           + " : " + drug.getGenericName()
            		           + " : " + drug.getNdc()
            		           );
    ...
    public static void main(String[] args) throws RPCException, OvidDomainException {
        FMDomainTestSuite suite = new FMDomainTestSuite();

        suite.setUp();
        try {
            suite.testFMQueryByIENWithAnnotatedClass();
        } finally {
            suite.tearDown();
        }
    }
