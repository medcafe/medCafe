...
(function($){
  $.fn.jScrollTouch = function (options) {
	...
	if((navigator.userAgent.match(/iPhone/i)) || (navigator.userAgent.match(/iPod/i)) || (navigator.userAgent.match(/iPad/i))) {
 		var isTouchScreen = 1;
 		{
 			console.log("jScrllTouch.js height " + options.height + " width " + options.width);
 		}
 	}else{
 		var isTouchScreen = 0;
 	}
 	$(this).css({'overflow': 'auto','position':'relative','height':options.height+'px', 'width':options.width +'px'});
	return this.each(function() {
		...
		if (isTouchScreen)
		{
			//console.log('jScrollTouch height of scrollable window ' + fullheight + ' width of scrollabel window ' + fullwidth);
		}
		...
		if(isTouchScreen){
			...
			scrollbarV.css({'display':'none','position':'absolute','width':'5px','height':fullheight+'px','left':cont.innerWidth()-7+'px','top':0,'background':'black','border':'1px white solid','-webkit-border-radius':'5px','opacity':'0.9'});
			
			var scrollbarH = $('<div></div>');
			scrollbarH.css({'display':'none','position':'absolute','height':'5px','width':fullwidth+'px','top':cont.innerHeight()-7+'px','left':0,'background':'black','border':'1px white solid','-webkit-border-radius':'5px','opacity':'0.9'});
