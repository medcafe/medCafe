...
public class PatientItemRepository extends OvidSecureRepository {
    ...
    private final VistaLinkConnection fmConnection;
    private String fmContext;

    public PatientItemRepository(VistaLinkConnection connection, VistaLinkConnection serverConnection, String fmContext) {
        super(connection);
        this.connection = connection;
        this.fmConnection = serverConnection;
    ...
    public Collection<IsAPatientItem> getNursingTasks(String patientDfn) throws OvidDomainException {
        ...
        try {
            rpcConnection = new VistaLinkRPCConnection(getConnection());
        ...
        try {
            ...
            Logger.getLogger(PatientItemRepository.class.getName()).log(Level.SEVERE, null, ex);
        } catch (RPCException ex) {
            Logger.getLogger(PatientItemRepository.class.getName()).log(Level.SEVERE, null, ex);
        }
        return collection;
    ...
    public Collection<IsAPatientItem> getAlerts(String patientDfn, String duz) throws OvidDomainException {
        Collection<IsAPatientItem> collection = new ArrayList<IsAPatientItem>();
        VistaLinkRPCConnection rpcConnection;
        try {
            rpcConnection = new VistaLinkRPCConnection(getConnection());
        } catch (OvidVistaLinkConnectionException e) {
            throw new OvidDomainException(e);
            ...
            for (int idx = 0; idx<items.length; ++idx) {
                if (items[idx].startsWith("Item=")) {
                    ...
                    try {
                        ...
                        Logger.getLogger(PatientItemRepository.class.getName()).log(Level.WARNING, null, ex);
                    }
    ...
    public Collection<IsAPatientItem> getProblems(String patientDfn) throws OvidDomainException{
        Collection<IsAPatientItem> collection = new ArrayList<IsAPatientItem>();
        VistaLinkRPCConnection rpcConnection;
        try {
            rpcConnection = new VistaLinkRPCConnection(getConnection());
        ...
        try {
            ...
            for (String item : items) {
                ...
                if (!parts[0].isEmpty()) {
                    ...
                    PatientAllergy a = new PatientAllergy(parts[0], parts[1]);
                    a.setReaction(parts[3]);
                    collection.add(a);
                }
            }
        } catch (RPCException ex) {
   ...
   public Collection<IsAPatientItem> getUnsignedOrdersByUser(String patientDfn, Date startDate, Date stopDate, String duz) throws OvidDomainException {
        ...
        try {
            ...
            for (int idx = 0; idx<items.length; ++idx) {
                if (items[idx].startsWith("Item=")) {
                    ...
                    if (item == null) {
                        java.util.logging.Logger.getLogger(PatientItemRepository.class.getName()).log(Level.WARNING, "Found Order= not preceded with Item= record");
                    } else {
    ...
    private boolean isEmptyResult(String[] items) {
    ...
    protected VistaLinkConnection getFMConnection() {
        return fmConnection;
    }
    ...
    public static void main(String[] args) throws OvidDomainException {
        ...
        try {
            ...
            // args = new String[] { "sql.medsphere.com", "8002", "DASHB1234", "DB1234!!"};
            if (args.length < 4) {
                System.err.println("usage: PatientList <host> <port> <access-code> <verify-code>");
                return;
            }

            conn = getDirectConnection(args[0], args[1], args[2], args[3]); // "ED1234", "SECRET1234!!"); // ("VL1234", "VL1234!!!");
            String duz = getDUZForLastConnection();
            if (conn==null) {
